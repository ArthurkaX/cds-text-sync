...
| `set-sync-folder [PATH] [--save]` | CLI -> daemon | Set `cds-sync-folder`; omit `PATH` for the saved project directory. `sync`, `./sync`, and `../sync` are project-relative. | automatic |
...
| `visu bind --svg FILE --elem N [--var|--text-var|--tap|--toggle|--color|--action NAME] [--clear]` | Patch only the binding `data-*` attributes on one already-drawn element — geometry, fill, stroke and gradients are untouched. The second-phase step after drawing, before recompiling with `from-svg --replace`. |
