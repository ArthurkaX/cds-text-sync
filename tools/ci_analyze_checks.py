...
def check_wheel_assets():
    ...
    with tempfile.TemporaryDirectory() as wheel_dir:
        ...
        if len(ctsrule) != 9:
            raise SystemExit(f"expected 9 .ctsrule in wheel, got {len(ctsrule)}")
        if len(docs) != 9:
            raise SystemExit(f"expected 9 .md in wheel, got {len(docs)}")
...
def check_wheel_install():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if ids != {"CTS0001", "CTS0002", "CTS0003", "CTS0004", "CTS0006", "CTS0007", "CTS0008", "CTS0009", "CTS0010"}:
            raise SystemExit(
                f"installed registry reports {sorted(ids)}; "
                "expected the nine human-analysis rules"
            )
        ...
        print("wheel install OK: venv entry point lists 9 human rules, selftest passes")
