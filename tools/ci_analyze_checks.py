...
def check_wheel_assets():
    ...
    with tempfile.TemporaryDirectory() as wheel_dir:
        ...
        if len(ctsrule) != 3:
            raise SystemExit(f"expected 3 .ctsrule in wheel, got {len(ctsrule)}")
        if len(docs) != 3:
            raise SystemExit(f"expected 3 .md in wheel, got {len(docs)}")
...
def check_wheel_install():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if ids != {"CTS0001", "CTS0002", "CTS0004"}:
            raise SystemExit(
                f"installed registry reports {sorted(ids)}; "
                "expected the three human-analysis rules"
            )
        ...
        print("wheel install OK: venv entry point lists 3 human rules, selftest passes")
